...
interface TemplateGalleryPageProps {
  /** Initial hash passed from parent router for SSR/static compatibility */
  initialHash?: string;
}
...
print(f"✨ Analysis complete! Generated {len(figures)} figures")`
  ...
  /**
   * Parse URL hash and auto-select template/section on mount
   * Supports:
   * - /#/templates/{category}/{template} - Specific template
   * - /#/templates#/{sectionId} - Expand specific section
   */
  useEffect(() => {
    if (typeof window === 'undefined') return;
    
    // Use initialHash from props or fall back to window.location.hash
    const hash = initialHash || window.location.hash;
    console.log('[SciCMP] Initial hash:', hash); // Debug log
    
    if (!hash || hash === '#landing' || hash === '#dashboard' || hash === '#templates' || hash === '#/templates') {
    ...
    if (sectionMatch) {
      ...
      console.log('[SciCMP] Section route detected:', sectionId);
      
      // Expand the requested section if it's valid
    ...
    setIsInitialized(true);
  }, [initialHash, initializePortalForTemplate]);
  
  /** Listen for hash changes and handle both template and section routing */
  useEffect(() => {
    if (typeof window === 'undefined') return;
    ...
    const handleHashChange = () => {
      ...
      if (!hash || hash === '#landing' || hash === '#dashboard') {
        ...
        // Collapse all sections when going to root
        setExpandedSections({
          'core-capabilities': false,
          'quick-start': false,
          'teaching-training': false,
      ...
      if (sectionMatch) {
        const sectionId = sectionMatch[1];
        console.log('[SciCMP] Section route from hashchange:', sectionId);
